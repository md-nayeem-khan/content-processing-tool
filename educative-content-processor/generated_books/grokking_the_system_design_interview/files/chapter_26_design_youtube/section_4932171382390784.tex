...
% Section Slug: evaluation-of-youtubes-design
% Generated: 2025-12-07T22:37:50.764341

\subsection{Requirements compliance}\label{jI6qhBMUAVP5yyxVPTw9i}
\\
...
\begin{enumerate}
\item
\protect\phantomsection\label{F2XITvDm3UDQ8Ldcz-ayd}
\textbf{Low latency/Smooth streaming} can be achieved through these strategies:
...
\begin{figure}[htbp]
 \centering
 \includegraphics[width=0.65\textwidth]{Images/chapter_26/section_4932171382390784/5748254579097600.png}
 \caption{How YouTube achieves scalability, availability, and good performance}
\end{figure}

\subsection{Trade-offs}\label{xs8AjrCRASniFQpbTMHwy}
\\
Let's discuss some of the trade-offs of our proposed solution.

\subsubsection{Consistency}\label{ETvj02phz4w4qRB8hcVaM}
\\
Our solution prefers high availability and low latency. However, strong consistency can take a hit because of high availability (see the \href{https://www.educative.io/edpresso/what-is-the-cap-theorem}{CAP theorem}). Nonetheless , for a system like YouTube, we can afford to let go of strong consistency. This is because we don't need to show a consistent feed to all the users. For example, different users subscribed to the same channel may not see a newly uploaded video at the same time. It
's important to mention that we'll maintain strong consistency of user data. This is another reason why we've decoupled user data from video metadata.

\subsubsection{Distributed cache}\label{-ShxDdYgydRlsyR5q7x3F}
\\
...
YouTube video access patterns are long-tailed, LRU-like algorithms are suitable for such data sets.

\subsubsection{Bigtable vs. MySQL}\label{sL68IcD9Vb8309Ou5gFFq}
\\
...
\\
The primary reason for the choice is performance and flexibility. The number of users on YouTube may not scale as much as the number of videos and thumbnails does. Moreover , we require storing the user and metadata in a structured form for convenient searching. Therefore, MySQL is a suitable choice for such cases.
\\
...
\subsubsection{Public versus private CDN}\label{FPhYiEeidNzRRs7EO9YcM}
\\
Our design relies on CDNs for low latency serving of the content. However, CDNs can be private(A CDN infrastructure that is for internal use only.) or public(A CDN infrastructure that is available to the public on a pay-as-you-use basis.). You Tube can choose between any one of the two options.
\\
...
\begin{enumerate}
\item
\protect\phantomsection\label{6sW3ovcC-UNHuFzZYNaXF}
Setting up a private CDN will require a lot of CAPEX(Capital expenditure is a significant expense used by a company to acquire a physical asset for the long term. The opposite is OPEX which refers to operational expenditure.).
\item
\protect\phantomsection\label{YT8wAR3t9eqWo8Bt9ZRgp}
For rather little viral traffic in certain regions, there will not be enough time to set up a new CDN.
\item
\protect\phantomsection\label{Yhg25Pk9q7vnTUdDKufPL}
...
\subsubsection{Duplicate videos}\label{igwDxzVTRjRH3YjEaBGsw}
\\
...
\subsection{Future scaling}\label{DUSt7xwZfnYcXt0x6sWHl}
\\
...
\begin{itemize}
\item
\protect\phantomsection\label{TNIG2-vsbC1Aw3H7gSiAI}
Web servers
\item
\protect\phantomsection\label{LXA8RloN6GZaIVKnMhaB5}
Application servers
\item
\protect\phantomsection\label{qVLvRYmBk1V5MwpHpF00j}
Datastores
\item
\protect\phantomsection\label{qABtMUgodV8mdV7XClNYs}
Placing load balancers among each of the layers above
\item
\protect\phantomsection\label{723Izu9SGVI0q8cfKLmNG}
...
\\
To resolve the problems above, YouTube has developed a solution called Vitess(Sougoumarane, Sugu, and Mike Solomon. ``Vitess: Scaling MySQL at YouTube Using Go.'' (2012).).

\begin{figure}[htbp]
 \centering
 \includegraphics[width=0.5\textwidth]{Images/chapter_26/section_4932171382390784/4768843109236736.png}
 \caption{Vitess system for scalability}
\end{figure}
...
\begin{figure}[htbp]
 \centering
 \includegraphics[width=0.5\textwidth]{Images/chapter_26/section_4932171382390784/4672221805281280.png}
 \caption{Vitess on the scalability versus structure spectrum}
\end{figure}
...
\subsection{Web server}\label{N9KzfGV1oHqCC0EX8C7OJ}
\\
A \textbf{web server} is an extremely important component and, with scale, a custom web server can be a viable solution. This is because most commercial or open-source solutions are general purpose and are developed with a wide range of users in mind. Therefore, a custom solution for such a successful service is desirable.
\\
...
\begin{quote}
\textbf{Note:} ESF is a custom web server developed by Google, and as of early 2022, it is widely in use in the Google ecosystem because out-of-the-box solutions were not enough for YouTube's needs.
\end{quote}
