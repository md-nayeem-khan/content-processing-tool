...
% Section ID: 6094096892690432
% Section Slug: evaluation-of-a-distributed-caches-design
% Generated: 2025-12-07T18:52:20.373783

\subsection{Requirements compliance}\label{_OYbt66lGf3C0LxA1BmHu}
\\
Let's evaluate our proposed design according to the design requirements.

\subsubsection{High performance}\label{U3PG1CHOCOhxeBed7vP_r}
\\
Here are some design choices we made that will contribute to overall good performance:

\begin{itemize}
\item
\protect\phantomsection\label{T66YeTpzdTvvIxEInCxld}
...
\item
\protect\phantomsection\label{2LApy7w3yErF5B5QLdH6d}
...
\end{itemize}
\begin{quote}
\textbf{Note:} A critical parameter for high performance is the selection of the eviction algorithm because the number of cache hits and misses depends on it. The higher the cache hit rate, the better the performance.
\end{quote}
\\
...
\begin{itemize}
\item
\protect\phantomsection\label{FfQmBa4RHyCiKy4GYEG27}
Cache hit service time (\protect\phantomsection\label{katex_inline_2aYpztGYLh-scV3AEJZos}{{{\(\text{99.9}^\text{{th}}\)}}} percentile): 5 ms
\item
\protect\phantomsection\label{LvyC8l0SAJTW-0Pa00Mqx}
Cache miss service time (\protect\phantomsection\label{katex_inline_3XDnKeXpN_GbSOo4UqjnO}{{{\(\text{99.9}^\text{{th}}\)}}} percentile): 30 ms (this includes time to get the data from the database and set the cache)
\end{itemize}
\\
...
\[
\text{EAT = Ratio}_{\text{hit}} \times \text{Time}_{\text{hit}} + \text{Ratio}_{\text{miss}} \times \text{Time}_{\text{miss}}
\]

Here, this means the following:
\\
\protect\phantomsection\label{katex_inline_any6yog2Og7OXxnuCvNZ-}{{{\(\text{EAT:}\)}}} Effective access time.
\\
\protect\phantomsection\label{katex_inline_RzaHTMrkRy41WAdDwn50y}{{{\(\text{Ratio}_{\text{hit}}:\)}}} The percentage of times a cache hit will occur.
\\
\protect\phantomsection\label{katex_inline_FGfeyNyz2yTJ77hSsLngD}{{{\(\text{Ratio}_{\text{miss}}:\)}}} The percentage of times a cache miss will occur.
\\
\protect\phantomsection\label{katex_inline_HOVRHykuBNvxK5ZbkrC7O}{{{\(\text{Time}_{\text{hit}}:\)}}} Time required to serve a cache hit.
\\
\protect\phantomsection\label{katex_inline_sGOgwRaKV8OOgONbMP0dt}{{{\( \text{Time}_{\text{miss}}:\)}}} Time required to serve a cache miss.
\\
...
\subsubsection{Scalability}\label{2EVbpxaOwwkUrXYFMDWGp}
\\
...
\begin{figure}[htbp]
 \centering
 \includegraphics[width=0.5\textwidth]{Images/chapter_16/section_6094096892690432/6550335900614656.png}
 \caption{Guarantees provided by a distributed cache}
\end{figure}

\subsubsection{High availability}\label{bPcupQGHdhVY3xgzSWolc}
\\
We have improved the availability through redundant cache servers. Redundancy adds a layer of reliability and fault tolerance to our design. We also used the \href{https://www.educative.io/courses/grokking-the-system-design-interview/data-replication\#Single-leader-/-primary-secondary-replication}{leader-follower algorithm} to conveniently manage a cluster shard. However, we haven't achieved high availability because we have two shard replicas, and at the moment, we assume that the replicas are within a data center.
\\
...
\subsubsection{Consistency}\label{PUEJC5M_6QAT8XojaiZ_j}
\\
It's possible to write data to cache servers in a synchronous or asynchronous mode. In the case of caching, the asynchronous mode is favored for improved performance. Consequently , our caching system suffers from inconsistencies. Alternatively
, strong consistency comes from synchronous writing, but this increases the overall latency, and the performance takes a hit.
\\
...
\subsubsection{Affordability}\label{yKV8N9u3Ied-y4N7pmjoa}
\\
Our proposed design has a low cost because it's feasible and practical to create such a system using commodity hardware.

\begin{quote}
\textbf{Quiz:}
\textbf{Question 1:} What happens if the leader node fails in the leader-follower protocol?
\textbf{Answer:} Two possible solutions can be used in that case: Use the leader election algorithm to elect a new leader among the set of available followers. Use a separate distributed configuration management service to monitor and select leaders.
\textbf{Question 2:} Is it a good idea to introduce synchronous replication to cache servers?
\textbf{Answer:} Synchronous replication on the cache clusters will introduce a performance penalty because it takes time to replicate the same data to all the cache servers. Therefore, there's a trade-off between performance and consistency in the usage of asynchronous versus synchronous replication, respectively. The answer depends on the use case. In case of cache servers, synchronous replication is a good choice if the replicated servers are within close proximity of the primary server.
\textbf{Question 3:} Assume that a large number of concurrent requests are forwarded to a single shard server. If the server uses the LRU algorithm, some of the requests will result in a cache miss and write a new entry to the cache,
...
\subsection{Summary}\label{3m7Bk-6hDFE6i5xmRBSyi}
\\
