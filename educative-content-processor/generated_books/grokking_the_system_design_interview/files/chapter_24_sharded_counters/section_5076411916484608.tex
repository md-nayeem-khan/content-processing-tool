...
% Section ID: 5076411916484608
% Section Slug: detailed-design-of-sharded-counters
% Generated: 2025-12-07T22:17:27.833299

\subsection{Detailed design}\label{_D8-culYLl5Ut1PX1Zoy1}
\\
...
\begin{itemize}
\item
\protect\phantomsection\label{cXBhxk8K-n1i6dEv-jG0e}
How many shards should be created against each new tweet?
\item
\protect\phantomsection\label{zrgAa1oDBEcsPF7eh2-rf}
How will the shard value be incremented for a specific tweet?
\item
\protect\phantomsection\label{SWYkMnhhaYK9FiRJibM6p}
What will happen in the system when read requests come from the end users?
\end{itemize}

\subsubsection{Sharded counter creation}\label{R9qO-QxyCl0hvHW-lnTXk}
\\
As we discussed earlier, when a user posts a tweet on Twitter, the \textbackslash{createCounter} API is called. The system creates multiple counters for each newly created post by the user. The following is the list of main counters created against each new tweet:

\begin{itemize}
\item
\protect\phantomsection\label{GrRtuZAnAbbcpZPGsv4Bb}
Tweet like counter
\item
\protect\phantomsection\label{LKg4Wfy-qtXKZtOL9EWoy}
Tweet reply counter
\item
\protect\phantomsection\label{qWgrY7LLOoGInEKN-kd-E}
Tweet retweet counter
\item
\protect\phantomsection\label{CHlwNL_VGXuOvXuYnh-ZJ}
...
\begin{quote}
\textbf{Quiz:}
\textbf{Question 1:} What happens when a user with just a few followers has a post go viral on Twitter?
\textbf{Answer:} The system needs to detect such cases where a counter unexpectedly starts getting very high write traffic. We\textquotesingle ll dynamically increase the number of shards of the affected counter to mitigate the situation.
\end{quote}

\subsubsection{Burst of writes requests}\label{JMm48bSlEh4KpQmwG25_S}
\\
...
\\
\paragraph{Round-robin selection}\label{Bvy9fUPOUD3T3_GO0O48a}
\\
...
\begin{figure}[htbp]
 ...
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/5971105287634944.png}
 \caption{The server assigns a write request to the first free shard, shard\_1, with a counter value of 12, but shard\_1 has not yet fulfilled its previous request, and it will increment its counter once the request is completely processed.}
 \end{subfigure}
 \hfill
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/4921116646047744.png}
 \caption{The server assigns another write request to the next free shard, shard\_2, with counter value 66, but shard\_2 has not yet fulfilled its previous request, and it will increment its counter once the request is completely processed.}
 \end{subfigure}
\end{figure}
...
\begin{figure}[htbp]
 ...
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/5860614787694592.png}
 \caption{The server assigns another write request to the next free shard, shard\_3, with counter value 31, but shard\_3 has not yet fulfilled its previous request, and it will increment its counter once the request is completely processed.}
 \end{subfigure}
 \hfill
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/4770008892047360.png}
 \caption{The server assigns another write request to the next free shard, shard\_4, with counter value 65, but shard\_4 has not yet fulfilled its previous request, and it will increment its counter once the request is completely processed.}
 \end{subfigure}
\end{figure}
...
\begin{figure}[htbp]
 ...
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/5740725255143424.png}
 \caption{The server can’t perform the next write request until shard\_1 is busy; though shard\_3 is done with incrementing its counter and is free, we can’t utilize it because of the round-robin pattern that we’re following here.}
 \end{subfigure}
\end{figure}

\paragraph{Random selection}\label{ZlTgJNUXZmwa32x12wnss}
\\
...
\\
\paragraph{Metrics-based selection}\label{lQzesm-W6jt5HsMfYgP3k}
\\
...
\begin{figure}[htbp]
 ...
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/6584999005782016.png}
 \caption{The user creates a tweet.}
 \end{subfigure}
 \hfill
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/6680853951676416.png}
 \caption{The user requests a counter using the create API.}
 \end{subfigure}
\end{figure}
...
\begin{figure}[htbp]
 ...
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/6084812265095168.png}
 \caption{The application server calculates the number of shards based on some metrics.}
 \end{subfigure}
 \hfill
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/6588704404930560.png}
 \caption{Requests to create shards for a specified counter using createCounter API are dispatched to geographically dispersed servers.}
 \end{subfigure}
\end{figure}

\subsubsection{Manage read requests}\label{xh5-tIhkiJt8BBx_Fw4fW}
\\
...
\begin{quote}
\textbf{Quiz:}
\textbf{Question 1:} Can you think of a use case where sharded counters with the above-mentioned consistency model might not be suitable?
\textbf{Answer:} We might not use sharded counters with a relaxed consistency model where we need strong consistency. An example can be read-then-write scenarios where we first need to get the accurate value of something before deciding to modify it (actually, such a scenario will need transaction support).
\end{quote}

\subsection{Using sharded counters for the Top K problem}\label{PMgs6Ojm-04tA_xtNnXB6}
\\
This section will discuss how we can use sharded counters to solve a real-world problem known as the \textbf{Top K} problem. We'll continue to use the real-time application Twitter as an example, where calculating trends is one of the Top K problems for Twitter. Here, KKK represents the number of top trends. Many users use various hashtags in their tweets. It is a huge challenge to manage millions of hashtags' counts to show them in individual users' trends timelines based on their locality.
\\
...
\begin{itemize}
\item
\protect\phantomsection\label{WyJeOURQQFmmuBDKoCCJt}
\textbf{Region-wise hashtag count} indicates the number of tweets with the same hashtag used within a specific geographical region. For example, thousands of tweets with the same tags from New York City suggest that users in the New York area may see this hashtag in their trends timeline.
\item
\protect\phantomsection\label{ol4phXoQfHpmVznwukyze}
A \textbf{time window} indicates the amount of time during which tweets with specific tags are posted.
\end{itemize}

\begin{figure}[htbp]
 \centering
 \includegraphics[width=0.5\textwidth]{Images/chapter_24/section_5076411916484608/5416338001887232.png}
 \caption{The system calculates the 24-hour count of the specified hashtag}
\end{figure}
...
\begin{itemize}
\item
\protect\phantomsection\label{mtgy5A1_0lLej5_AKt6p0}
The \textbf{global hashtag counter} represents the total of all location-based counters.
\item
\protect\phantomsection\label{g0fwx-d48Yehr7IgnT8XZ}
\textbf{Location-based counters} represent their current count when the system reaches the set threshold in a specified time and the hashtag becomes a trend for some users. For example, Twitter sets 10,000 as a threshold. When location-based hashtag counts reach 10,000, Twitter shows these hashtags in the trends timeline of the users of the respective country where the hashtag is being used. The specified hashtag may be displayed worldwide if counts increase in all countries.
\end{itemize}
\\
...
\begin{quote}
\textbf{Note:} Twitter also uses other metrics to optimize Top K selection, but we've discussed the leading metrics here.
\end{quote}
\subsection{Placement of sharded counters}\label{vEeZeqQPS4ove4VqQMR1n}
\\
\hfill\break
...
\begin{figure}[htbp]
 ...
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/4970083007201280.png}
 \caption{The users send write requests to the application server.}
 \end{subfigure}
 \hfill
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/4726154634592256.png}
 \caption{The application server identifies the appropriate counters against tweets in the Redis key-value store.}
 \end{subfigure}
\end{figure}
...
\begin{figure}[htbp]
 ...
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/6699189528821760.png}
 \caption{The application server randomly chooses the shards against counters in the Redis key-value store.}
 \end{subfigure}
 \hfill
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/5701104224829440.png}
 \caption{The selected shards are highlighted. }
 \end{subfigure}
\end{figure}
...
\begin{figure}[htbp]
 ...
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/5568483100590080.png}
 \caption{The sum of the values of all shards is computed.}
 \end{subfigure}
 \hfill
 \begin{subfigure}[b]{0.48\textwidth}
 \centering
 \includegraphics[width=\textwidth]{Images/chapter_24/section_5076411916484608/6339959101718528.png}
 \caption{All the computed sums are stored in the Cassandra store.}
 \end{subfigure}
\end{figure}

\subsection{Requirements compliance}\label{vhkJhqM52FpFmi-8hkjHO}
\\
This section will evaluate the sharded counters and explain how sharded counters will increase performance by providing high availability, scalability, and reliability.

\subsubsection{Availability}\label{u9tc_rj3Gf7HCj7zsMDvO}
\\
...
\subsubsection{Scalability}\label{AA9V9vtCzkgfmMZbDd_UX}
\\
...
\subsubsection{Reliability}\label{m-8k_I0uUD2349tEIGqtY}
\\
...
\subsection{Conclusion}\label{N9Vqas3xLNaMlGbTgNab1}
\\
